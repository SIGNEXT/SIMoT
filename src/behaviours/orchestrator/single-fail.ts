import OrchestratorBehaviour from "./orchestratorBehaviour";

const DEFAULT_FAIL = 1000;
const DEFAULT_BOOT_TIME = 2000;

class SingleFailOrchBehaviour extends OrchestratorBehaviour {
    ...
    override run(): void {
        ...
        setTimeout(() => {
            this.orchestrator?.stop(this.memError);
            this.hasFailed = true;
            if (this.bootTime > 0) {
                setTimeout(() => {
                    this.orchestrator?.reset(this.orchestrator.nodes)
    ...
    static fromJSON(jsonData: { failIn: number, bootTime: number, memError: boolean }) {
        return new SingleFailOrchBehaviour(jsonData.failIn, jsonData.bootTime, jsonData.memError);
    }
}

export { SingleFailOrchBehaviour };
