...
class CbbaAuction {
  ...
  public async run(): Promise<[boolean, boolean, Node[]]> {
    ...
    while(unchanged < this.UNCHANGED_COUNT && !this.stopped) {
      // console.log(`Building ${this.round} - ${unchanged} - ${this.incomingBuffer.size}`)
  ...
  */
  ...
  private update(j: number, beliefs: TaskBidInfo) {
    ...
    this.logger.info(`(${this.internalID}) Updated task ${j}-Sender:${beliefs.senderId}-Z:${beliefs.z}-Y:${beliefs.y}-T:${beliefs.t}`)
