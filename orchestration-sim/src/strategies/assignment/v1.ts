...
class AssignmentStrategyV1 extends AssignmentStrategy {
    ...
    private getBestDevice(node: Node, devices: EnhancedDeviceData[], currentAssignment: NodeDeviceAssignment): EnhancedDeviceData | null {
        ...
        let bestDevice: EnhancedDeviceData | null = null;

        for (const device of devices) {
            const nrNodes = currentAssignment.get(device.id)!.length;
            ...
            if (device.memoryErrorNodes && (nrNodes + 1) >= device.memoryErrorNodes) {
                continue;
            }
