...
export default (version: string) => {
  ...
  const devices = [
    ...
    new V1BullyDevice(6, { capabilities: ["t1", "t2", "t3"], totalRam: 150 }, version),
    new V1BullyDevice(7, { capabilities: ["t1", "t2", "t3"], totalRam: 200 }, version),
  ];
  return new Scenario("trivial-nofail", nodes, devices, () => null, []);
};
